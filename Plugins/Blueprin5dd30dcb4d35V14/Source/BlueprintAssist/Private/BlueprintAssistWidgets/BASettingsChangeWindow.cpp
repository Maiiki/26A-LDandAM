...
void SBASettingsChangeWindow::SaveSettings(EBASettingsLocation Location)
{
	...
	for (auto Set : Settings)
	{
		...
		if (!bSuccess)
		{
			UE_LOG(LogTemp, Warning, TEXT("[%hs] Failed to save setting %s to %s"), __FUNCTION__, *GetNameSafe(Set), *IniPath);
...
TSharedRef<SWidget> SBASettingsChangeWindow::MakeSaveButton(EBASettingsLocation Location)
{
	...
	return SNew(SButton)
		...
		.Text(FText::Format(INVTEXT("Save to {Loc}"), {SettingLocationText}))
